...
class Album: Codable {
    ...
    required init(from decoder: Decoder) throws {
        ...
        if container.contains(.covertArt) {
            var covertArtArray = try container.nestedUnkeyedContainer(forKey: .covertArt)
            while !covertArtArray.isAtEnd {
                let covertArtDictionary = try covertArtArray.nestedContainer(keyedBy: CodingKeys.CovertArtKeys.self)
                let url = try covertArtDictionary.decode(URL.self, forKey: .url)
                covertArt.append(url)
            }
        }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        var coverArtContainer = container.nestedUnkeyedContainer(forKey: .covertArt)
